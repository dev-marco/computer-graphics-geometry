...
namespace Geometry {
    ...
    void Plane::calcForX (void) {
        float_t norm = -1.0 / this->normal[0];
    ...
    void Plane::calcForY (void) {
        float_t norm = -1.0 / this->normal[1];
    ...
    void Plane::calcForZ (void) {
        float_t norm = -1.0 / this->normal[2];
    ...
    Plane::Plane (const Vec<3> &_normal, float_t _d) :
    ...
    bool Plane::param (const Vec<3> &point, float_t &s, float_t &t, bool in) {
    ...
    bool Plane::intersectLine (const Line &line, Vec<3> &normal, float_t &t_inter, bool fix_normal) {
